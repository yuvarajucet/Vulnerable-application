...
public class InfoDisclosureController : Controller
{
    ...
    [HttpPost("vulnerableuserlogin")]
    public async Task<IActionResult> VulnerableLoginUser()
    {
    ...
    [HttpGet("user")]
    public IActionResult VulnerableUserProfile()
    {
        ...
        return View("Vulnerable/User");
    }

    [HttpGet("accountinfo")]
    public IActionResult VulnerableAccountInfo()
    {
        return View("Vulnerable/AccountInfo");
    }

    [HttpGet("submitform")]
    public IActionResult VulnerableAccountInfoResult(CardDetails model)
    {
        return View("Vulnerable/AccountInfoResult", model);
    }
    

    [HttpPost]
    public IActionResult VulnerableGetAccountInformation()
    {
        var loginInfo = Request.ReadFormAsync().Result;
        var client = new HttpClient();
        client.DefaultRequestHeaders.Add("Authorization", "DlgcmqKTGUiwPspQirEwcbMlps");
        var response = client.GetAsync($"http://{Request.Host.ToString()}/vulnerable/infodisclosure/api/GetAccountInfo?username={loginInfo["username"]}").Result;
