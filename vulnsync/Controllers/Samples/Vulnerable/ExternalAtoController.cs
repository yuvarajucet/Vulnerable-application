...
public class ExternalAtoController : Controller
{
    ...
    {
        this._connectionString = $"DataSource={Path.Combine(Directory.GetCurrentDirectory(), "Database/ExternalAto/VulnerableDatabase.db")}";
        this.SeedUsers();
    }
    
    [HttpGet]
    public IActionResult VulnerableExternalAto()
    {
    ...
    /*----------------------------- SSO handler -------------------------------------------*/
    [HttpGet("/vulnerable/oauth/login")]
    public IActionResult VulnerableOAuthLogin(string state, string code)
    {
        return View("Vulnerable/OAuthLogin");
    }
