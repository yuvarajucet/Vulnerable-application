## Vulnerable Example for CSRF Attack

#### Explanation:
- Without a CSRF token, an attacker can craft a malicious form on another site to submit a request to this endpoint on behalf of the user.
- The lack of CSRF protection means the application cannot distinguish between legitimate and forged requests.

#### Vulnerable Code Example (ASP.NET Core)

**Controller Action**: No CSRF validation is applied.
```csharp
...
**HTML Form**: No CSRF token is included, leaving it open to exploitation.
```html
...
<form id="csrf-form" action="http://localhost:5187/Vulnerable/Csrf/Updatepassword" method="POST" style="display:none;">
